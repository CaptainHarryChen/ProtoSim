...
class MultiGridMPMScene : public SimulationScene<Real>
{
public:
...
int main()
{
    ...
    auto scene = std::make_shared<MultiGridMPMScene<Real>>();
    scene->SetupScene();
    app->AddObject(scene);
    ...
    auto solver = std::make_shared<MultiGridMPMSolver<Real>>(scene->m_object_types, scene->m_particle_object_id,

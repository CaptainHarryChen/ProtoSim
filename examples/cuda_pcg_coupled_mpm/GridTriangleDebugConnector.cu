...
namespace GridTriangleDebugConnectorKernel
{
    ...
    __global__ void transfer_particle_data(viewer::Particle *dev_particles, PCGCoupledMPMSolverData<Real> data, unsigned int num_particle)
    {
        ...
        unsigned int z = id % mpm.m_grid_size[2];
        unsigned int y = (id / mpm.m_grid_size[2]) % mpm.m_grid_size[1];
        unsigned int x = id / mpm.m_grid_size[2] / mpm.m_grid_size[1];
        dev_particles[id].Position.x = mpm.m_outer_bbox[0] + x * mpm.m_grid_spacing;
        dev_particles[id].Position.y = mpm.m_outer_bbox[1] + y * mpm.m_grid_spacing;
        dev_particles[id].Position.z = mpm.m_outer_bbox[2] + z * mpm.m_grid_spacing;
        float dis;
        bool inside;
        unsigned int idx;
    ...
    __global__ void transfer_line_seg_data(viewer::LineSeg *dev_line_segs, PCGCoupledMPMSolverData<Real> data, unsigned int num_particle)
    {
        ...
        unsigned int y = (id / mpm.m_grid_size[2]) % mpm.m_grid_size[1];
        unsigned int x = id / mpm.m_grid_size[2] / mpm.m_grid_size[1];
        dev_line_segs[id].a.x = mpm.m_outer_bbox[0] + x * mpm.m_grid_spacing;
        dev_line_segs[id].a.y = mpm.m_outer_bbox[1] + y * mpm.m_grid_spacing;
        dev_line_segs[id].a.z = mpm.m_outer_bbox[2] + z * mpm.m_grid_spacing;
        float dis;
        bool inside;
        unsigned int idx;
        CPICTools::unpack_tri_info(data.dev_grid_tri_info[id], dis, inside, idx);
...
template <typename Real>
GridTriangleDebugConnector<Real>::GridTriangleDebugConnector(std::shared_ptr<viewer::ParticleBatch> particles, std::shared_ptr<viewer::LineSegment> line_segs, PCGCoupledMPMSolverData<Real> data)
    : m_particles(particles), m_line_segs(line_segs), m_data(data)
{
...
void GridTriangleDebugConnector<Real>::TransferData()
{
    ...
    GridTriangleDebugConnectorKernel::transfer_particle_data<Real><<<CUDA_GRID_SIZE(num_particle), CUDA_BLOCK_SIZE>>>(dev_particles, m_data, num_particle);
    ...
    GridTriangleDebugConnectorKernel::transfer_line_seg_data<Real><<<CUDA_GRID_SIZE(num_particle), CUDA_BLOCK_SIZE>>>(dev_line_segs, m_data, num_particle);

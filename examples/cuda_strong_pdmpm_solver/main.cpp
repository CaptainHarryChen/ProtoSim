...
#include <Solver/StrongPDMPMSolver.cuh>
#include <proj_config.h>

template <typename Real>
class StrongPDMPMScene : public SimulationScene<Real>
{
public:
    ...
    {
        auto solver = std::dynamic_pointer_cast<StrongPDMPMSolver<Real>>(this->m_solver);
        assert(solver != nullptr && "Solver must be StrongPDMPMSolver");
...
int main()
{
    ...
    auto app = GLFWApp::GetInstance("Strong Coupled PD-MPM Solver Example", 1600, 900);

    auto scene = std::make_shared<StrongPDMPMScene<Real>>();
    scene->SetupScene();
    app->AddObject(scene);
    ...
    auto solver = std::make_shared<StrongPDMPMSolver<Real>>(
        scene->m_positions,

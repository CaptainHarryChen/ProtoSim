...
namespace IQMPMSolverKernel
{
    ...
    template <typename Real>
    __global__ void G2P_velocity(IQMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle * data->m_num_object)
            return;
        unsigned int object_id = i / data->m_num_particle;
        unsigned int particle_id = i % data->m_num_particle;
        if (data->dev_object_type[object_id] == MPM_STATIC)
            return;
        Real *particle_position = &data->dev_particle_position[particle_id * 3];
        unsigned int leftbottom_grid_id = data->dev_particle_to_grid_id[particle_id];
        unsigned int x, y, z;
        get_grid_xyz(x, y, z, leftbottom_grid_id, data);
        Real G2P_vel[3] = {0.0f, 0.0f, 0.0f};
        data->dev_G2P_mass[i] = 0;
        Real *normal = &data->dev_G2P_normal[i * 3];
        normal[0] = normal[1] = normal[2] = 0.0f;
        for (unsigned int dx = 0; dx < 3; ++dx)
            for (unsigned int dy = 0; dy < 3; ++dy)
                for (unsigned int dz = 0; dz < 3; ++dz)
                {
                    unsigned int grid_id = get_grid_id(object_id, x + dx, y + dy, z + dz, data);
                    Real grid_position[3];
                    get_grid_position(grid_position, grid_id, data);
                    Real weight = grid_particle_quadratic_weight(grid_position, particle_position, data->m_grid_spacing);
                    Real delta_position[3];
                    ...
                    cudaPhysics::axpby(normal, (Real)1.0, normal, weight * data->dev_grid_mass[grid_id], delta_position, 3);
                    cudaPhysics::axpby(G2P_vel, (Real)1.0, G2P_vel, weight, &data->dev_grid_velocity[grid_id * 3], 3);
                    data->dev_G2P_mass[i] += weight * data->dev_grid_mass[grid_id];
                }
        cudaPhysics::norm3InPlace(normal);
        data->dev_G2P_velocity[i] = cudaPhysics::dot3(G2P_vel, normal);
    }

    template <typename Real>
    __global__ void G2P_coupling(IQMPMSolverData<Real> *data, unsigned int total_iter)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
        unsigned int object_id = data->dev_particle_object_id[i];
        if (data->dev_object_type[object_id] == MPM_STATIC)
            return;
        Real *particle_position = &data->dev_particle_position[i * 3];
        unsigned int leftbottom_grid_id = data->dev_particle_to_grid_id[i];
        ...
        for (unsigned int dx = 0; dx < 3; ++dx)
            for (unsigned int dy = 0; dy < 3; ++dy)
                for (unsigned int dz = 0; dz < 3; ++dz)
                {
                    ...
                    get_grid_position(grid_position, grid_id, data);
                    Real weight = grid_particle_quadratic_weight(grid_position, particle_position, data->m_grid_spacing);
                    cudaPhysics::axpby(&data->dev_particle_velocity[i * 3], (Real)1.0, &data->dev_particle_velocity[i * 3], weight, &data->dev_grid_velocity[grid_id * 3], 3);
                }
        for (unsigned int iter = 0; iter < total_iter; iter++)
        {
            for (unsigned int j = 0; j < data->m_num_object; j++)
            {
                if (j == object_id)
                    continue;
                Real &G2P_vel = data->dev_G2P_velocity[j * data->m_num_particle + i];
                Real vel_p = cudaPhysics::dot3(&data->dev_particle_velocity[i * 3], &data->dev_G2P_normal[j * data->m_num_particle * 3 + i * 3]);
                if (G2P_vel - vel_p >= 0.0f)
                    continue;
                // completely inelastic collision
                Real vel_res = (data->dev_G2P_mass[j * data->m_num_particle + i] * G2P_vel + data->dev_particle_mass[i] * vel_p) / (data->dev_G2P_mass[j * data->m_num_particle + i] + data->dev_particle_mass[i]); 
                data->dev_G2P_delta_vel[i] += vel_res - G2P_vel;
                G2P_vel = vel_res;
                Real delta_vel[3];
                cudaPhysics::vecMul3(delta_vel, vel_res - vel_p, &data->dev_G2P_normal[j * data->m_num_particle * 3 + i * 3]);
                cudaPhysics::vecAdd3(&data->dev_particle_velocity[i * 3], &data->dev_particle_velocity[i * 3], delta_vel);
            }
        }
    }

    template <typename Real>
    __global__ void G2P_C(IQMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle * data->m_num_object)
            return;
        ...
        for (unsigned int dx = 0; dx < 3; ++dx)
            for (unsigned int dy = 0; dy < 3; ++dy)
                for (unsigned int dz = 0; dz < 3; ++dz)
                {
                    ...
                    cudaPhysics::vecSubs3(delta_position, grid_position, particle_position);
                    Real grid_vel[3];
                    if (object_id == data->dev_particle_object_id[particle_id])
                        cudaPhysics::vecCopy3(grid_vel, &data->dev_grid_velocity[grid_id * 3]);
                    else
                    {
                        cudaPhysics::vecCopy3(grid_vel, &data->dev_particle_velocity[particle_id * 3]);
                        // Real grid_vel_dot_normal = cudaPhysics::dot3(&data->dev_grid_velocity[grid_id * 3], &data->dev_G2P_normal[i * 3]);
                        // cudaPhysics::axpby(grid_vel, (Real)1.0f, &data->dev_grid_velocity[grid_id * 3], data->dev_G2P_velocity[i] - grid_vel_dot_normal, &data->dev_G2P_normal[i * 3], 3);
                        // cudaPhysics::axpby(grid_vel, (Real)1.0f, &data->dev_grid_velocity[grid_id * 3], data->dev_G2P_delta_vel, &data->dev_G2P_normal[i * 3], 3);
                    }
                    Real temp_C[9];
                    cudaPhysics::vecvecT(temp_C, grid_vel, delta_position, 3, 3);
                    cudaPhysics::matMul3(temp_C, weight * 4 / data->m_grid_spacing / data->m_grid_spacing, temp_C);
                    for (unsigned int j = 0; j < 9; j++)
                        atomicAdd(&data->dev_particle_C[particle_id * 9 + j], temp_C[j]);
                }
    }
    ...
    std::vector<Real> bbox, Real grid_spacing, unsigned int boundary_thickness)
{
    ...
    cudaMalloc(&m_data.dev_G2P_velocity, sizeof(Real) * m_data.m_num_particle * m_data.m_num_object);
    cudaMalloc(&m_data.dev_G2P_delta_vel, sizeof(Real) * m_data.m_num_particle * m_data.m_num_object);
    cudaMalloc(&m_data.dev_G2P_mass, sizeof(Real) * m_data.m_num_particle * m_data.m_num_object);
    cudaMalloc(&m_data.dev_G2P_normal, sizeof(Real) * m_data.m_num_particle * m_data.m_num_object * 3);
...
IQMPMSolver<Real>::~IQMPMSolver()
{
    ...
    cudaFree(m_data.dev_G2P_velocity);
    cudaFree(m_data.dev_G2P_delta_vel);
    cudaFree(m_data.dev_G2P_mass);
    cudaFree(m_data.dev_G2P_normal);
...
void IQMPMSolver<Real>::Step()
{
    ...
    cudaMemset(m_data.dev_particle_velocity, 0, sizeof(Real) * m_data.m_num_particle * 3);
    cudaMemset(m_data.dev_particle_C, 0, sizeof(Real) * m_data.m_num_particle * 9);
    cudaMemset(m_data.dev_G2P_delta_vel, 0, sizeof(Real) * m_data.m_num_particle * m_data.m_num_object);

    IQMPMSolverKernel::G2P_velocity<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle * m_data.m_num_object), CUDA_BLOCK_SIZE>>>(m_dev_data);
    IQMPMSolverKernel::G2P_coupling<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data, 10);
    IQMPMSolverKernel::G2P_C<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle * m_data.m_num_object), CUDA_BLOCK_SIZE>>>(m_dev_data);

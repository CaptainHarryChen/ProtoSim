...
public:
	...
	SurfaceMesh(std::vector<Vertex> vertices, std::vector<unsigned int> indices, std::vector<glm::vec3> material, bool cubic = true)
	{
		...
		setupMesh();
		if (cubic) shader = std::make_shared<Shader>("pbr", true);
		else shader = std::make_shared<Shader>("pbr2D", true);
	...
	SurfaceMesh(std::vector<Vertex> vertices, std::vector<unsigned int> indices, std::vector<std::string> textures, bool cubic = true)
	{
		...
		setupMesh();
		if (cubic) shader = std::make_shared<Shader>("pbr_texture", true);
		else shader = std::make_shared<Shader>("pbr_texture2D", true);
		shaderEdge = std::make_shared<Shader>("edge");
	}
	...
	)
	{
		...
		if (textureBinded == true)
		{
			...
			{
				glActiveTexture(GL_TEXTURE5 + i); glBindTexture(GL_TEXTURE_2D, depthMap2D[i]);
			}
			shader->setInt("albedoMap", 0);
			shader->setInt("normalMap", 1);
			shader->setInt("metallicMap", 2);
		...
		else
		{
			...
			for (int i = 0; i < depthMap2D.size(); ++i)
			{
				glActiveTexture(GL_TEXTURE0 + i); glBindTexture(GL_TEXTURE_2D, depthMap2D[i]);
			}
			shader->setVec3("albedoIn", material[0]);
			shader->setFloat("metallicIn", material[1][0]);
			shader->setFloat("roughnessIn", material[1][1]);

...
namespace StrongPDMPMSolverKernel
{
    ...
    __global__ void update_F(StrongPDMPMSolverData<Real> *data)
    {
    ...
    template <typename Real>
    __global__ void calc_particle_to_leftbottom_grid_id(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void P2G_momentum_and_mass_and_K(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void grids_gravity(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_grid)
            return;
    ...
    template <typename Real>
    __global__ void particles_gravity(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void G2P_calc_particle_temp_F(StrongPDMPMSolverData<Real> *data, Real alpha = 0)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void grid_op(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_grid)
            return;
    ...
    template <typename Real>
    __global__ void calc_particle_internal_energy(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void calc_grid_op_energy(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_grid)
            return;
    ...
    template <typename Real>
    __global__ void grids_boundary_conditions(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_grid)
            return;
    ...
    template <typename Real>
    __global__ void G2P_velocity_and_C(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void update_particle_positions(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void particles_boundary_conditions(StrongPDMPMSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
    ...
    template <typename Real>
    __global__ void swap_answer(StrongPDMPMSolverData<Real> *data)
    {
...
void StrongPDMPMSolver<Real>::Step()
{
    ...
    cudaMemset(m_data.dev_grid_mass, 0, sizeof(Real) * m_data.m_num_grid);
    cudaMemset(m_data.dev_grid_diag_K, 0, sizeof(Real) * m_data.m_num_grid * 3);
    StrongPDMPMSolverKernel::update_F<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
    StrongPDMPMSolverKernel::particles_gravity<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
    StrongPDMPMSolverKernel::calc_particle_to_leftbottom_grid_id<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
    StrongPDMPMSolverKernel::P2G_momentum_and_mass_and_K<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
    ...
    cudaMemset(m_data.dev_particle_velocity, 0, sizeof(Real) * m_data.m_num_particle * 3);
    cudaMemset(m_data.dev_particle_C, 0, sizeof(Real) * m_data.m_num_particle * 9);
    StrongPDMPMSolverKernel::G2P_velocity_and_C<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
    StrongPDMPMSolverKernel::update_particle_positions<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
    // StrongPDMPMSolverKernel::particles_boundary_conditions<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
}
...
template <typename Real>
Real *StrongPDMPMSolver<Real>::GetDeviceParticlePositions()
{
    return m_data.dev_particle_position;
}

template <typename Real>
void StrongPDMPMSolver<Real>::NewtonSolver()
{
    ...
    {
        cudaMemset(m_data.dev_grid_force, 0, sizeof(Real) * m_data.m_num_grid * 3);
        StrongPDMPMSolverKernel::G2P_calc_particle_temp_F<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
        StrongPDMPMSolverKernel::P2G_calc_grid_force<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
        StrongPDMPMSolverKernel::grid_op<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data);
        StrongPDMPMSolverKernel::jacobi_iteration<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data);
        ...
        StrongPDMPMSolverKernel::grids_boundary_conditions<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data);
        SwapAnswerBuffers();
    }
}

template <typename Real>
void StrongPDMPMSolver<Real>::NewtonWithLineSearch()
{
    cudaMemcpy(m_data.dev_grid_velocity_hat, m_data.dev_grid_velocity, sizeof(Real) * m_data.m_num_grid * 3, cudaMemcpyDeviceToDevice);
    cudaMemcpy(m_data.dev_grid_velocity_prev, m_data.dev_grid_velocity, sizeof(Real) * m_data.m_num_grid * 3, cudaMemcpyDeviceToDevice);
    for (unsigned int iter = 0; iter < MAX_ITERATIONS; ++iter)
    {
        ...
        *m_data.u_residual = 0;
        ...
        if (iter % LINE_SEARCH_ITER == 0)
        {
            ...
            StrongPDMPMSolverKernel::calc_particle_internal_energy<<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
            StrongPDMPMSolverKernel::calc_grid_op_energy<<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data);
            cudaDeviceSynchronize();
            ...
            while (alpha > MIN_ALPHA)
            {
                *m_data.u_energy = 0;
                ...
                StrongPDMPMSolverKernel::G2P_calc_particle_temp_F<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data, alpha);
                StrongPDMPMSolverKernel::calc_particle_internal_energy<<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_dev_data);
                StrongPDMPMSolverKernel::calc_grid_op_energy<<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data);
                cudaDeviceSynchronize();
                if (m_verbose)
...
template <typename Real>
void StrongPDMPMSolver<Real>::ChebyshevSolver()
{
    cudaMemcpy(m_data.dev_grid_velocity_hat, m_data.dev_grid_velocity, sizeof(Real) * m_data.m_num_grid * 3, cudaMemcpyDeviceToDevice);
    cudaMemcpy(m_data.dev_grid_velocity_prev, m_data.dev_grid_velocity, sizeof(Real) * m_data.m_num_grid * 3, cudaMemcpyDeviceToDevice);
    Real omega = 1;
    for (unsigned int iter = 0; iter < MAX_ITERATIONS; ++iter)
    {
        ...
        StrongPDMPMSolverKernel::Chebyshev<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data, omega);
        SwapAnswerBuffers();
    }
    if (m_verbose)
    {
...
template <typename Real>
void StrongPDMPMSolver<Real>::SwapAnswerBuffers()
{
    ...
    StrongPDMPMSolverKernel::swap_answer<Real><<<1, 1>>>(m_dev_data);
}

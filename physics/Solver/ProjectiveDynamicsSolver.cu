...
namespace ProjectiveDynamicsSolverKernel
{
    ...
    __global__ void initial_guess(ProjectiveDynamicsSolverData<Real> data)
    {
        ...
        if (v >= data.m_num_vert)
            return;
        cudaPhysics::axpby(&data.dev_velocity[3 * v], (Real)1.0, &data.dev_velocity[3 * v], data.m_time_step, data.m_gravity, 3);
...
ProjectiveDynamicsSolver<Real>::ProjectiveDynamicsSolver(const std::vector<Real> &position, const std::vector<unsigned int> &tetrahedron, const std::vector<Real> &tetrahedron_density)
{
    ...
    m_data.m_gravity[0] = 0.0f;
    m_data.m_gravity[1] = -GRAVITY;
    m_data.m_gravity[2] = 0.0f;

    ProjectiveDynamicsSolverKernel::tetrahedron_initialize<Real><<<CUDA_GRID_SIZE(m_data.m_num_tet), CUDA_BLOCK_SIZE>>>(m_data);
    ProjectiveDynamicsSolverKernel::tet_stiffness_matrix_diag<Real><<<CUDA_GRID_SIZE(m_data.m_num_tet), CUDA_BLOCK_SIZE>>>(m_data);
}
...
void ProjectiveDynamicsSolver<Real>::Step()
{
    ...
    ProjectiveDynamicsSolverKernel::initial_guess<Real><<<CUDA_GRID_SIZE(m_data.m_num_vert), CUDA_BLOCK_SIZE>>>(m_data);
    ...
    for (unsigned int iter = 0; iter < MAX_ITERATIONS; ++iter)
    {
        ...
        ProjectiveDynamicsSolverKernel::calc_tetrahedron_force<Real><<<CUDA_GRID_SIZE(m_data.m_num_tet), CUDA_BLOCK_SIZE>>>(m_data);
        ProjectiveDynamicsSolverKernel::accumulate_vert_force<Real><<<CUDA_GRID_SIZE(m_data.m_num_tet), CUDA_BLOCK_SIZE>>>(m_data);
        ProjectiveDynamicsSolverKernel::calc_ground_collision_force<Real><<<CUDA_GRID_SIZE(m_data.m_num_vert), CUDA_BLOCK_SIZE>>>(m_data);
        ProjectiveDynamicsSolverKernel::jacobi_iteration<Real><<<CUDA_GRID_SIZE(m_data.m_num_vert), CUDA_BLOCK_SIZE>>>(m_data);
        UpdateChebyshevOmega(omega, iter);
        ProjectiveDynamicsSolverKernel::Chebyshev<Real><<<CUDA_GRID_SIZE(m_data.m_num_vert), CUDA_BLOCK_SIZE>>>(m_data, omega);
        SwapPositionBuffers();
    }
    ProjectiveDynamicsSolverKernel::update_velocity<Real><<<CUDA_GRID_SIZE(m_data.m_num_vert), CUDA_BLOCK_SIZE>>>(m_data);
}
...
void ProjectiveDynamicsSolver<Real>::SwapPositionBuffers()
{
    ...
    ProjectiveDynamicsSolverKernel::swap_position<Real><<<1, 1>>>(m_data);
}

...
namespace MultiGridMPMSolverKernel
{
    ...
    __global__ void calc_particle_to_leftbottom_grid_id(MultiGridMPMSolverData<Real> data)
    {
        ...
        data.dev_particle_to_grid_id[i] = (x * data.m_grid_size[1] * data.m_grid_size[2] + y * data.m_grid_size[2] + z) + (data.dev_particle_object_id[i] * data.m_grid_size[0] * data.m_grid_size[1] * data.m_grid_size[2]);
    }
    ...
    {
        id = id % (data.m_grid_size[0] * data.m_grid_size[1] * data.m_grid_size[2]);
        z = id % data.m_grid_size[2];
        y = (id / data.m_grid_size[2]) % data.m_grid_size[1];
        x = id / data.m_grid_size[2] / data.m_grid_size[1];
    }
    ...
    {
        return (x * data.m_grid_size[1] * data.m_grid_size[2] + y * data.m_grid_size[2] + z) + (object_id * data.m_grid_size[0] * data.m_grid_size[1] * data.m_grid_size[2]);
    }
    ...
    __global__ void grids_couple(MultiGridMPMSolverData<Real> data)
    {
        ...
        if (grid_id >= data.m_grid_size[0] * data.m_grid_size[1] * data.m_grid_size[2])
            return;
        for (unsigned int i = 0; i < data.m_num_object; i++)
        {
            unsigned int grid_id_i = grid_id + i * data.m_grid_size[0] * data.m_grid_size[1] * data.m_grid_size[2];
            if (data.dev_grid_mass[grid_id_i] <= 0)
                continue;
            for (unsigned int j = i + 1; j < data.m_num_object; j++)
            {
                unsigned int grid_id_j = grid_id + j * data.m_grid_size[0] * data.m_grid_size[1] * data.m_grid_size[2];
    ...
    __global__ void grids_boundary_conditions(MultiGridMPMSolverData<Real> data)
    {
        ...
        if (x >= data.m_grid_size[0] - 1 - data.m_boundary_thickness)
        ...
        if (y >= data.m_grid_size[1] - 1 - data.m_boundary_thickness)
        ...
        if (z >= data.m_grid_size[2] - 1 - data.m_boundary_thickness)
    ...
    std::vector<Real> bbox, Real grid_spacing, unsigned int boundary_thickness)
{
    ...
    printf("grid_size = %u x [%u %u %u]\n", m_data.m_num_object, m_data.m_grid_size[0], m_data.m_grid_size[1], m_data.m_grid_size[2]);
    for (unsigned int i = 0; i < 3; ++i)
        m_data.m_outer_bbox[i + 3] = m_data.m_outer_bbox[i] + m_data.m_grid_spacing * m_data.m_grid_size[i];
    m_data.m_num_grid = m_data.m_num_object * m_data.m_grid_size[0] * m_data.m_grid_size[1] * m_data.m_grid_size[2];
    printf("num_grid = %u\n", m_data.m_num_grid);
    ...
    m_data.m_gravity[0] = 0.;
    m_data.m_gravity[1] = -GRAVITY;
    m_data.m_gravity[2] = 0.;
...
void MultiGridMPMSolver<Real>::Step()
{
    ...
    cudaMemset(m_data.dev_grid_momentum, 0, sizeof(Real) * m_data.m_num_grid * 3);
    cudaMemset(m_data.dev_grid_mass, 0, sizeof(Real) * m_data.m_num_grid);
    MultiGridMPMSolverKernel::update_F<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    // MultiGridMPMSolverKernel::particles_gravity<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    MultiGridMPMSolverKernel::calc_particle_affine_momentum<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    MultiGridMPMSolverKernel::calc_particle_to_leftbottom_grid_id<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    MultiGridMPMSolverKernel::P2G_momentum_and_mass<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);

    cudaMemset(m_data.dev_grid_normal, 0, sizeof(Real) * m_data.m_num_grid * 3);
    MultiGridMPMSolverKernel::P2G_normal_estimate<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    MultiGridMPMSolverKernel::grid_normal_normalize<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_data);

    MultiGridMPMSolverKernel::calc_grids_velocity<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_data);
    MultiGridMPMSolverKernel::grids_gravity<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_data);
    for(unsigned int it = 0; it < 10; ++it)
        MultiGridMPMSolverKernel::grids_couple<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid / m_data.m_num_object), CUDA_BLOCK_SIZE>>>(m_data);
    MultiGridMPMSolverKernel::grids_boundary_conditions<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_data);
    cudaMemset(m_data.dev_particle_velocity, 0, sizeof(Real) * m_data.m_num_particle * 3);
    cudaMemset(m_data.dev_particle_C, 0, sizeof(Real) * m_data.m_num_particle * 9);
    MultiGridMPMSolverKernel::G2P_velocity_and_C<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    
    // cudaMemset(m_data.dev_max_particle_velocity, 0, sizeof(Real));
    // MultiGridMPMSolverKernel::get_max_particle_velocity<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    // Real max_velocity;
    ...
    MultiGridMPMSolverKernel::update_particle_positions<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
    // MultiGridMPMSolverKernel::particles_boundary_conditions<Real><<<CUDA_GRID_SIZE(m_data.m_num_particle), CUDA_BLOCK_SIZE>>>(m_data);
}

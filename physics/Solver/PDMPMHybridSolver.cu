...
namespace PDMPMHybridSolverKernel
{
    ...
    template <typename Real>
    __global__ void G2P_couple_PD(PDMPMHybridSolverData<Real> *data)
    {
        unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i >= data->m_num_particle)
            return;
...
void PDMPMHybridSolver<Real>::Step()
{
    ...
    PDMPMHybridSolverKernel::G2P_couple_PD<Real><<<CUDA_GRID_SIZE(m_data.m_num_grid), CUDA_BLOCK_SIZE>>>(m_dev_data);
